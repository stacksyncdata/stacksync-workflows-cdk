...
templates/            The scaffold the CLI copies for `stacksync workflow-connector create`.
...
| `templates/connector` | **copied** into the new repo by `stacksync workflow-connector create` | the developer's machine |
...
one CDK line in its `requirements.txt`, or with `stacksync workflow-connector cdk <ref>`:
...
`stacksync workflow-connector run` prints the installed version and the ref you track, and
